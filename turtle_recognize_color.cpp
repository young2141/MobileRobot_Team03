...
void sendMessage(const sensor_msgs::ImageConstPtr &msg){
	...
	int resultRed = detectColor(red, img_frame);
	int resultBlue = detectColor(blue, img_frame);
	int resultYellow = detectColor(yellow, img_frame);

	if(resultRed + resultBlue + resultYellow == 1) {
		if(resultBlue == 1) {
			cout << "<<blue detected>>" << endl;
			/*geometry_msgs::Twist baseCmd;
            baseCmd.linear.x=1.0;
            baseCmd.linear.y=1.0;
            baseCmd.linear.z=1.0;
			pub.publish(baseCmd);*/
			return;
		}
		else if(resultRed == 1) {
		...
		else if(resultYellow == 1) {
			cout << "<<yellow detected>>" << endl;
			return;
		}
	}
	else if(resultRed + resultBlue + resultYellow == 0){
		cout << "<<not detected>>" << endl;
	}
	else {
		cout << "<<multiple detect>>" << endl;
	}
}
